...
struct ContentView: View {
    ...
    var gameQuestions: [Question] {
        get {
            let questionCount = Int(questionCounts[selectedQuestionCount])
            return Array(questions.choose(questionCount))
        }
        set {
            gameQuestions = newValue
        }
    }
    ...
    var body: some View {
        VStack {
            if !isStartGame {
                ...
                VStack {
                    Text("Question \(currentQuestion+1)/\(Int(questionCounts[selectedQuestionCount]) ?? 100).")
                        .font(.largeTitle)
                    Group {
    ...
    private func checkAnswer() {
        ...
        if currentQuestion == gameQuestions.count - 1 {
            ...
            if (Int(answer) ?? 0) == current.answer {
                correctAnswer += 1
                currentQuestion += 1
            } else {
                wrongAnswer += 1
